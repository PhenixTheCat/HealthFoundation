...
include('./modele/requests.online_psychotest.php');
...
switch ($function) {
    ...
    if(isset($_POST['inscriptionSuite'])) {
        ...
        echo $structure;
        $code=uniqid(true);
        $data = array($firstName,$lastName,$userType,$email,$password,$birthdate,$instructor,$structure,$address,$city,$postCode,$country,$phoneNumber,$code);
    ...
    case 'modificationMotDePasse':
        ...
        if(isset($_POST['piloteModifMdp'])){
              if (!empty($_POST['Nmdp']) AND isPassword($_POST['Cmdp']) AND isPassword($_POST['Amdp'])) {
                ...
                $user = getOneUser($database,$_SESSION['userID']);
