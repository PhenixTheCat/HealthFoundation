...
for($i=0;$i<$_SESSION["nbCriteria"];$i++) {
	...
	?>
	<input id="search" class="criteriaSearch" name="Text<?php echo($i)?>" type="text" placeholder=<?php echo _("Rechercher");?> value = "<?php echo($_SESSION['criteriaText'][$i]) ?>" />
	<?php if($i == $_SESSION["nbCriteria"]-1) {?>
	
	<input class="criteriasButton" id="Boutton" type="button" value=<?php echo _("Ajouter un critère");?> onclick="showCriterias(1,<?php echo($_SESSION["nbCriteria"]);?>,false,<?php echo($i)?>)" />
	<?php }?>
	<?php if($i != $_SESSION["nbCriteria"]-1) {?>
	<input class="criteriasButton" id="Boutton" type="button" value=<?php echo _("Retirer le critère");?> onclick="showCriterias(-1,<?php echo($_SESSION["nbCriteria"]);?>,false,<?php echo($i)?>)" />
	<?php }?>
