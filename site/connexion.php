...
if(isset($_POST['Connexion'])) {
  ...
  $reqPilot = $bdd->query('SELECT id FROM user WHERE email=\''.$mail.'\' AND password =\''.$mdp.'\'AND type =\''.Pilot.'\'');
  $reqInstructor = $bdd->query('SELECT id FROM user WHERE email=\''.$mail.'\' AND password =\''.$mdp.'\'AND type =\''.Instructor.'\'');
  $reqAdmin = $bdd->query('SELECT id FROM user WHERE email=\''.$mail.'\' AND password =\''.$mdp.'\'AND type =\''.Administrator.'\'');
	if($donnee = $reqPilot->fetch())
	{
		$_SESSION['isConnected'] = true;
		$_SESSION['userID'] = $donnee['id'];
		header('Location:indexPilot.php');
  }
  elseif($donnee = $reqInstructor->fetch())
	{
		$_SESSION['isConnected'] = true;
		$_SESSION['userID'] = $donnee['id'];
		header('Location:indexInstructor.php');
  }
  elseif($donnee = $reqAdmin->fetch())
	{
		$_SESSION['isConnected'] = true;
		$_SESSION['userID'] = $donnee['id'];
